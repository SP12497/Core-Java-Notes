/*
Promise:
    - Promises in JavaScript are objects that represent the eventual completion
        or failure of an asynchronous operation and its resulting value.
    - They provide a way to handle asynchronous operations in a 
        more elegant and structured manner compared to traditional callback-based approaches.
    ...
    - Methods of promises:
        1. then(onFulfilled, onRejected) :
            - The then() method is used to specify what to do when the promise is fulfilled (resolved successfully). 
    ...
    - Promise Combinators:
        ...
        4. Promise.any()
*/
console.log("_______#1_______");
// let val = Promise.resolve("Hi");
// console.log("Promise.resolve:", val); // Promise { 'Hi' }

// val = Promise.resolve("Hi").then((message)=>{console.log("message:", message);})    // message: Hi
// console.log(val);   // Promise { <pending> }

// Promise.reject("Bye").then((message) => {console.log(message)}).catch((error) => {console.log("catch", error)});      // catch bye


console.log("_______#2_______");
...
//--- chain of promises
// importantAction("SP World").then((message) => {             // resolve
//     console.log("resolved:", message);
//     likeTheVideo("JS Course").then((message) => {           // resolve
//         console.log("resolved:", message);
//         shareTheVideo("JS Course").then((message) => {      // resolve
//             console.log("resolved:", message);
//             deleteTheVideo("JS Course").then((message) => { // reject
//                 console.log("resolved:", message);
//             }).catch((error) => {
...
// importantAction("Mr.Coder_sp")                  // resolve
//     .then((message) => {
//         console.log("then1", message);
//         return likeTheVideo("JS Course");       // resolve
//     })
//     .then((message) => {
//         console.log("then2", message);
//         return shareTheVideo("JS Course");      // resolve
//     })
//     .then((message) => {
//         console.log("then3", message);
//         return deleteTheVideo("JS Course");     // reject
//     })
//     .then((message) => {
...
Promise Combinators:
    ...
    1. Promise.all():
        - It will execute in concurrent, not parallely.
            (JS can't execute code in parallely, it can execute sequentially or concurrently.)
...
//         console.log("resolved - " + messages[1]);

//     })
//     .catch(messages => console.log("rejected - " + messages));

/*
resolved - Subscribe to SP world,Liked  the Java,Shared the Java
resolved - object
resolved - Liked  the Java
*/
...
async function myAsyncAllFunction() {
    try {
        ...
        let [sub, share] = await Promise.all([
            asyncImportantAction("asyncImportantAction SP"),
            asyncShareTheVideo("asyncShareTheVideo Java"),
...
[
  { status: 'fulfilled', value: 'Subscribe to SP world' },
  { status: 'fulfilled', value: 'Shared the Java' },
  { status: 'rejected', reason: 'Failed to delete the MySQL' }
]
*/
...
async function myAsyncRaceFunction() {
    try {
        let results = await Promise.race([
            asyncImportantAction("SP world1"),       // 1000 msec
            asyncShareTheVideo("Java1"),             // 800 msec
        ]);
...
async function myAsyncAnyFunction() {
    try {
        let results = await Promise.any([
            asyncImportantAction("SP world1"),   // 1000 ms
            asyncShareTheVideo("Java1"),  // 800 ms
        ]);
        console.log(results);       // Shared the Java1

        let results2 = await Promise.any([
            deleteTheVideo("SP world2"),    // 900 ms : rejected
            asyncImportantAction("Java2"),  // 1000 ms : resolved
        ]);
        console.log(results2);              // Subscribe to Java2
    ...
    await Promise.any([
        deleteTheVideo("SP world"),     // 1000 ms  // reject
        downloadTheVideo("Java"),       // 800 ms   // reject
    ]).then((message) => {
        console.log(message);
    }).catch((err) => {
        console.log("rejected :", err);
...
function promRecurse(funcPendingPromises) {
    if (funcPendingPromises.length === 0) return;
    // const arr = [11,22,33]
    // const val = arr.shift(); // val = 11, arr = [22,33]
    const currPromise = funcPendingPromises.shift();
    currPromise
        .then((res) => console.log("resolve:", res))
        .catch((res) => console.log("rejected:", res))

    promRecurse(funcPendingPromises);
}

// promRecurse([
//     asyncImportantAction("#SP world"),      // promise are called here. and pending state of promises are passed to promRecurse function.
//     asyncShareTheVideo("#Java"),
//     deleteTheVideo("#MySQL")
// ]);
/*
resolve: Shared the #Java
rejected: Failed to delete the #MySQL
resolve: Subscribe to #SP world
*/
