...
class ABC 
{
	...
	static public ABC getInstance()		//Double-Checked Locking
	{
		if(obj==null) 	//check1
		{
			//multiple thread runs same time so it happens that multiple getInstance method calls same time so multiple obj is created.  So overcome this problem we use synchronized function
